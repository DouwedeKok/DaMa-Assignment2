...
def create_vectorizer_unigram(files):
    return CountVectorizer(input=files)


def create_vectorizer_bigram(files):
    return CountVectorizer(input=files, analyzer='word', ngram_range=(2, 2))
...
if __name__ == '__main__':
    ...
    try:
        vec_uni = create_vectorizer_unigram(corpus_dec_train)
    except:
        print('')
    feature_counts_uni = create_feature_counts(vec_uni, corpus_dec_train)
    features_uni = create_features(vec_uni)

    vec_bi = create_vectorizer_bigram(corpus_dec_train)
    feature_counts_bi = create_feature_counts(vec_bi, corpus_dec_train)
    features_bi = create_features(vec_bi)
